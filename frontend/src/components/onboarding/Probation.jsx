...
import { Users, Clock, CheckCircle, RefreshCw, Plus, ChevronLeft, ChevronRight, MoreHorizontal } from 'lucide-react';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip, Label } from 'recharts';

const kpiData = [
  { title: 'Employees on Probation', value: '18', icon: <Users size={20} color="#2952E3" />, bgColor: '#EFF6FF' },
  { title: 'Due for Review', value: '8', icon: <Clock size={20} color="#F59E0B" />, bgColor: '#FFFBEB' },
  { title: 'Confirmed', value: '10', icon: <CheckCircle size={20} color="#10B981" />, bgColor: '#ECFDF5' },
  { title: 'Extended Probation', value: '2', icon: <RefreshCw size={20} color="#8B5CF6" />, bgColor: '#F5F3FF' },
];

const probationData = [
  { id: 'EMP001', name: 'Rahul Sharma', dept: 'Engineering', joined: '20 Feb 2024', end: '20 May 2024', manager: 'Arjun Mehta', status: 'Due for Review' },
  { id: 'EMP002', name: 'Priya Patel', dept: 'Human Resources', joined: '18 Feb 2024', end: '18 May 2024', manager: 'Sneha Kapoor', status: 'Due for Review' },
  { id: 'EMP003', name: 'Amit Kumar', dept: 'Design', joined: '15 Feb 2024', end: '15 May 2024', manager: 'Rohan Verma', status: 'Confirmed' },
  { id: 'EMP004', name: 'Neha Singh', dept: 'Finance', joined: '14 Feb 2024', end: '14 May 2024', manager: 'Vikram Singh', status: 'Confirmed' },
  { id: 'EMP005', name: 'Vikas Yadav', dept: 'Marketing', joined: '10 Feb 2024', end: '10 May 2024', manager: 'Anjali Desai', status: 'Extended' },
  { id: 'EMP006', name: 'Pooja Joshi', dept: 'Sales', joined: '08 Feb 2024', end: '08 May 2024', manager: 'Karan Malhotra', status: 'Due for Review' },
];

const pieData = [
  { name: 'Confirmed', value: 10, color: '#2952E3' },
  { name: 'Due for Review', value: 8, color: '#10B981' },
  { name: 'Extended', value: 2, color: '#EF4444' },
];

const timelineData = [
  { month: 'May 2024', count: 8 },
  { month: 'June 2024', count: 6 },
  { month: 'July 2024', count: 4 },
];
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ background: '#F8FAFC' }}>
                  ...
                  <th style={{ padding: '16px 24px', fontSize: '12px', fontWeight: '600', color: '#64748B', borderBottom: '1px solid #F1F5F9' }}>Joined On</th>
              ...
              <tbody>
                {probationData.map((row, index) => (
                  <tr key={row.id} style={{ borderBottom: index === probationData.length - 1 ? 'none' : '1px solid #F8FAFC', transition: 'background 0.2s', ':hover': { background: '#F8FAFC' } }}>
                    ...
                    <td style={{ padding: '16px 24px', fontSize: '13px', color: '#475569', whiteSpace: 'nowrap' }}>{row.joined}</td>
                    <td style={{ padding: '16px 24px', fontSize: '13px', color: '#1E293B', fontWeight: '500', whiteSpace: 'nowrap' }}>{row.end}</td>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={pieData} innerRadius={40} outerRadius={55} paddingAngle={2} dataKey="value" cx="50%" cy="50%" stroke="none">
                      ...
                      <Label value="18" position="center" fill="#1E293B" style={{ fontSize: '24px', fontWeight: '700' }} />
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
                ...
                {pieData.map((item, idx) => (
                  <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      ...
                      <span style={{ color: '#94A3B8' }}>({Math.round((item.value/18)*100)}%)</span>
                    </div>
                  </div>
                ))}
              </div>
