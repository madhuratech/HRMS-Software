...
import { Search, Plus, Edit2, Trash2, Calendar, DollarSign, ChevronRight, CheckCircle, Clock, XCircle, FileText } from 'lucide-react-native';
...
const EXPENSE_SCHEMA = [
  ...
  { key: 'department_name', label: 'Department', type: 'select', options: ['IT', 'HR', 'Operations', 'Sales', 'Marketing'] },
  { key: 'amount', label: 'Amount', keyboardType: 'numeric', required: true },
  { key: 'date', label: 'Date (YYYY-MM-DD)', required: true },
  { key: 'status', label: 'Status', type: 'select', options: ['Pending', 'Approved', 'Rejected'] }
];

export default function ExpenseClaimsScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.warn('Error fetching expense claims:', error);
      setData([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Claim', `Are you sure you want to delete this expense claim?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/expenses/claims/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if (updatedItem.id) {
        await apiClient.put(`/expenses/claims/${updatedItem.id}`, updatedItem);
      } else {
        await apiClient.post('/expenses/claims', updatedItem);
      }
    } catch(err) {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case 'Approved': return { bg: '#DCFCE7', text: '#15803D', icon: CheckCircle };
      ...
      <View style={styles.card}>
        ...
        <View style={styles.detailsGrid}>
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText} numberOfLines={1}>{item.category_name}</Text>
          </View>
          <View style={styles.detailItem}>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            ...
            <Text style={styles.headerSubtitle}>Manage employee expenses</Text>
          </View>
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Create Claim' : actionModalMode === 'edit' ? 'Edit Claim' : 'Claim Details'}
        />
      )}
    </View>
  );
