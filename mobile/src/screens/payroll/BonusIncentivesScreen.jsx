...
export default function BonusIncentivesScreen({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error fetching BonusIncentives:', err);
  ...
  const renderItem = ({ item }) => {
    const amount = `₹ ${parseFloat(item.amount || 0).toLocaleString('en-IN')}`;
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <View style={styles.cardInfo}>
            <Text style={styles.cardTitle}>{item.employeeName || 'Employee'}</Text>
            <Text style={styles.cardSubtitle}>{item.type || 'Performance Bonus'}</Text>
          </View>
          <View style={[styles.statusBadge, { backgroundColor: item.status === 'Approved' ? '#DEF7EC' : item.status === 'Pending' ? '#FEF3C7' : '#FEECDC' }]}>
            <Text style={[styles.statusText, { color: item.status === 'Approved' ? '#03543F' : item.status === 'Pending' ? '#92400E' : '#8A2C0D' }]}>
        ...
        <View style={styles.cardDetails}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={styles.detailValue}>{item.rating || '5/5'}</Text>
          </View>
          <View style={styles.detailRow}>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <View>
          <Text style={styles.headerTitle}>Bonus & Incentives</Text>
          <Text style={styles.headerSubtitle}>Manage employee bonuses</Text>
        </View>
