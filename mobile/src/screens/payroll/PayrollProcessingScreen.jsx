...
export default function PayrollProcessingScreen({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error fetching PayrollProcessing:', err);
  ...
  const renderItem = ({ item }) => {
    const gross = item.gross_amount ? `₹ ${(item.gross_amount / 100000).toFixed(2)}L` : '₹ 0.0L';
    const net = item.net_amount ? `₹ ${(item.net_amount / 100000).toFixed(2)}L` : '₹ 0.0L';
    const date = item.run_date ? new Date(item.run_date).toLocaleDateString() : 'N/A';
    
    return (
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <View style={styles.cardInfo}>
            <Text style={styles.cardTitle}>{item.period || 'Payroll Run'}</Text>
            <Text style={styles.cardSubtitle}>Date: {date}</Text>
          </View>
          <View style={[styles.statusBadge, { backgroundColor: item.status === 'Completed' ? '#DEF7EC' : '#FEECDC' }]}>
            <Text style={[styles.statusText, { color: item.status === 'Completed' ? '#03543F' : '#8A2C0D' }]}>
              {item.status || 'Pending'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.cardDetails}>
          <View style={styles.detailRow}>
            ...
            <Text style={styles.detailValue}>{gross}</Text>
          </View>
          <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>Net Amount</Text>
            <Text style={styles.detailValue}>{net}</Text>
          </View>
          <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>Processed</Text>
            <Text style={styles.detailValue}>{item.processed_employees || 0} / {item.total_employees || 0}</Text>
          </View>
        </View>
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.headerSubtitle}>Process monthly payroll</Text>
        </View>
