...
export default function SalaryStructureScreen({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error fetching SalaryStructure:', err);
  ...
  const renderItem = ({ item }) => {
    const amount = item.total_ctc 
      ? `₹ ${Number(item.total_ctc).toLocaleString('en-IN')}${item.frequency === 'Hourly' ? '/hr' : ''}`
      : '₹ 85,000';
    const date = item.created_at ? new Date(item.created_at).toLocaleDateString() : '01 Apr 2024';
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <View style={styles.cardInfo}>
            ...
            <Text style={styles.cardSubtitle}>{item.code || 'CODE123'}</Text>
          </View>
        ...
        <View style={styles.cardDetails}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={styles.detailValue}>{item.frequency || item.freq || 'Monthly'}</Text>
          </View>
          <View style={styles.detailRow}>
            <Text style={styles.detailLabel}>Employees</Text>
            <Text style={styles.detailValue}>{item.employees || 0}</Text>
          </View>
          <View style={styles.detailRow}>
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.headerSubtitle}>Manage salary structures</Text>
        </View>
