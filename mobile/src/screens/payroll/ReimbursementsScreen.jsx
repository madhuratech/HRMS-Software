  ...
  const fetchData = async () => {
    try {
      ...
      if (extractedList && extractedList.length > 0) {
        ...
        setData([
          { id: 1, title: 'Sample Reimbursements 1', status: 'Active', date: new Date().toISOString().split('T')[0] },
          { id: 2, title: 'Sample Reimbursements 2', status: 'Pending', date: new Date().toISOString().split('T')[0] }
        ]);
      }
    } catch (error) {
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete', 'Are you sure you want to delete this item?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             if(item._id || item.id) {
               await apiClient.delete(`/payroll/${item._id || item.id}`);
             }
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if(updatedItem._id || updatedItem.id) {
        await apiClient.put(`/payroll/${updatedItem._id || updatedItem.id}`, updatedItem);
      } else {
        await apiClient.post('/payroll', updatedItem);
      }
    } catch(err) {
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#1E293B', '#0F172A']} style={styles.headerGradient}>
        ...
        <View style={styles.searchContainer}>
          ...
          <Text style={styles.searchPlaceholder}>Search in Reimbursements...</Text>
        </View>
      </LinearGradient>
