...
import { Plus, Search, CheckCircle, Clock, AlertTriangle, TrendingUp, Edit2, Trash2 } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import apiClient from '../../api/client';
import ActionModals from '../../components/common/ActionModals';

export default function AppraisalsScreen({ navigation }) {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [refreshing, setRefreshing] = useState(false);
  const [kpiData, setKpiData] = useState({ total: 0, approved: 0, pending: 0, rejected: 0 });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [dashboardRes, appraisalsRes] = await Promise.all([
        apiClient.get('/appraisals/dashboard').catch(() => null),
        apiClient.get('/appraisals?page=1&limit=50').catch(() => null)
      ]);
      ...
      if (appraisalsRes?.data?.success && appraisalsRes.data.data.appraisals) {
        setData(appraisalsRes.data.data.appraisals);
      } else {
        setData([]);
      }
    } catch (error) {
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Appraisal', `Are you sure you want to delete this appraisal for ${item.employee_name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/app/appraisals/${item.id}`);
             fetchData();
          } catch(err) {
             Alert.alert('Error', 'Failed to delete appraisal');
          }
      }}
    ]);
  };
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      const payload = {
        ...
        current_salary: parseFloat(updatedItem.current_salary || updatedItem.currentSalary) || 0,
        proposed_salary: parseFloat(updatedItem.proposed_salary || updatedItem.proposedSalary) || 0,
        effective_date: updatedItem.effective_date || updatedItem.effectiveDate,
        appraisal_percentage: parseFloat(updatedItem.appraisal_percentage || updatedItem.appraisalPercentage) || 0,
        remarks: updatedItem.remarks || '',
        status: updatedItem.status || 'In Progress'
      };

      if (updatedItem.id) {
        await apiClient.put(`/app/appraisals/${updatedItem.id}`, payload);
      } else {
        await apiClient.post('/app/appraisals', payload);
      }
    } catch(err) {
  ...
  const SCHEMA = [
    ...
    { key: 'current_salary', label: 'Current Salary', type: 'text', keyboardType: 'numeric', required: true },
    { key: 'proposed_salary', label: 'Proposed Salary', type: 'text', keyboardType: 'numeric', required: true },
    { key: 'appraisal_percentage', label: 'Appraisal Percentage (%)', type: 'text', keyboardType: 'numeric' },
    { key: 'effective_date', label: 'Effective Date (YYYY-MM-DD)', type: 'text', required: true },
    { key: 'status', label: 'Status', type: 'select', options: ['In Progress', 'Approved', 'Rejected'] },
    ...
    <View style={styles.kpiContainer}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.kpiScroll}>
        ...
        <View style={styles.kpiCard}>
          ...
          <Text style={styles.kpiValue}>{kpiData.rejected || 0}</Text>
        </View>
      </ScrollView>
    </View>
  );
  ...
  const renderItem = ({ item }) => {
    ...
    // Formatting currency
    const current = Number(item.current_salary || 0).toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 });
    const proposed = Number(item.proposed_salary || 0).toLocaleString('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 });
      ...
      <View style={styles.card}>
        ...
        <View style={styles.salarySection}>
          <View style={styles.salaryCol}>
            ...
            <Text style={styles.salaryValue}>{current}</Text>
          </View>
          <View style={styles.salaryDivider} />
          <View style={styles.salaryCol}>
            <Text style={styles.salaryLabel}>Proposed Salary</Text>
            <Text style={[styles.salaryValue, { color: '#10B981' }]}>{proposed}</Text>
          </View>
        </View>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            ...
            <Text style={styles.headerSubtitle}>Manage salary hikes & appraisals</Text>
          </View>
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Create Appraisal' : 'Edit Appraisal'}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  salarySection: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  salaryCol: { flex: 1 },
  salaryLabel: { fontSize: 12, color: '#64748B', marginBottom: 4 },
  salaryValue: { fontSize: 15, fontWeight: '700', color: '#1E293B' },
  salaryDivider: { width: 1, height: 24, backgroundColor: '#E2E8F0', marginHorizontal: 16 },
