...
import { Search, Plus, Target, User, X } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import apiClient from '../../api/client';

export default function KRAsScreen() {
  ...
  const fetchKRAs = async () => {
    try {
      setLoading(true);
      const res = await apiClient.get('/performance/kras');
  ...
  const handleAddKRA = async () => {
    if (!employeeId || !title) {
      Alert.alert('Error', 'Employee ID and Title are required.');
      return;
    }
    try {
      setSubmitting(true);
      await apiClient.post('/performance/kras', { 
        employee_id: parseInt(employeeId), title, description 
      });
      setEmployeeId(''); setTitle(''); setDescription('');
      setModalVisible(false);
      fetchKRAs();
    } catch (err) {
      console.error('Error adding KRA:', err);
      Alert.alert('Error', 'Failed to add KRA.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerTextContainer}>
          ...
          <Text style={styles.headerSubtitle}>Key Result Areas</Text>
        </View>
        <TouchableOpacity style={styles.addButton} onPress={() => setModalVisible(true)}>
          <LinearGradient colors={['#10B981', '#059669']} style={styles.gradientBtn}>
            <Plus size={18} color="#FFF" />
            <Text style={styles.addButtonText}>Add KRA</Text>
          </LinearGradient>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Set New KRA</Text>
            ...
            <View style={styles.modalBody}>
              ...
              <TextInput style={styles.modalInput} placeholder="KRA Title" placeholderTextColor="#94A3B8" value={title} onChangeText={setTitle} />
              <TextInput 
                ...
                multiline 
              />
              <TouchableOpacity style={[styles.submitButton, submitting && { opacity: 0.7 }]} onPress={handleAddKRA} disabled={submitting}>
                <Text style={styles.submitButtonText}>{submitting ? 'Saving...' : 'Set KRA'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
