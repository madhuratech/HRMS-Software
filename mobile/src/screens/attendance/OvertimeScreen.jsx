...
import { Clock, ChevronLeft, Plus, X, CheckCircle, XCircle, Eye, Edit2, Trash2 } from 'lucide-react-native';
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete', 'Are you sure you want to delete this item?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
            await apiClient.delete(`/attendance/overtime/${item.id || item._id}`);
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if (updatedItem.id || updatedItem._id) {
        await apiClient.put(`/attendance/overtime/${updatedItem.id || updatedItem._id}`, updatedItem);
      }
