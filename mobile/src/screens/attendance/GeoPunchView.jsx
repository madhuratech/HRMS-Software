    ...
    <View style={s.empCard}>
      <View style={s.empCardTop}>
        ...
        <View style={s.empCardRight}>
          ...
          <TouchableOpacity style={s.viewBtn} > <Eye size={14} color='#2563EB' />
...
// ─── Main Screen ─────────────────────────────────────────────────────────────
export default function GeoPunchView() {
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete', 'Are you sure you want to delete this item?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
            await apiClient.delete(`/attendance/today-status/${item.id || item._id}`);
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if (updatedItem.id || updatedItem._id) {
        await apiClient.put(`/attendance/today-status/${updatedItem.id || updatedItem._id}`, updatedItem);
      }
