...
import { Download, CalendarCheck, CalendarDays, CalendarX, Clock } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';

const SUMMARY_DATA = [
  { id: 1, dept: 'Engineering', total: 45, taken: 120, pending: 5, remaining: 800 },
  { id: 2, dept: 'Sales', total: 20, taken: 60, pending: 2, remaining: 350 },
  { id: 3, dept: 'HR', total: 5, taken: 15, pending: 0, remaining: 90 }
];
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#1E293B', '#0F172A']} style={styles.headerGradient}>
        <View style={styles.headerContent}>
          <View>
            <Text style={styles.headerTitle}>Leave Reports</Text>
            <Text style={styles.headerSubtitle}>Monitor time off & balances</Text>
          </View>
        ...
        <View style={styles.kpiRow}>
          <View style={styles.kpiCardFull}>
            <View style={styles.kpiRowHeader}>
              <Text style={styles.kpiLabel}>Total Leaves Taken</Text>
              <CalendarCheck size={16} color="#2563EB" />
            </View>
            <Text style={styles.kpiValueLarge}>195</Text>
          </View>
        </View>

        <View style={styles.kpiRow}>
          {renderKpi('Sick', '85', CalendarX, '#EF4444', '#FEE2E2')}
          {renderKpi('Casual', '110', CalendarDays, '#10B981', '#D1FAE5')}
        </View>
        
        <View style={styles.kpiRow}>
          {renderKpi('Pending', '7', Clock, '#F59E0B', '#FEF3C7')}
          {renderKpi('Avg/Emp', '1.5', CalendarDays, '#6366F1', '#E0E7FF')}
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Department Balances</Text>
          
          {SUMMARY_DATA.map(item => (
            <View key={item.id} style={styles.summaryCard}>
              <View style={styles.summaryHeader}>
                <Text style={styles.summaryDept}>{item.dept}</Text>
                <Text style={styles.summaryTotal}>{item.total} Emp</Text>
              </View>
              <View style={styles.summaryDetails}>
                <Text style={styles.summaryText}>Taken: {item.taken}</Text>
                <Text style={styles.summaryText}>Pending: {item.pending}</Text>
                <Text style={styles.summaryText}>Remaining: {item.remaining}</Text>
              </View>
            </View>
          ))}
        </View>
