...
import { Search, Plus, Trash2, FileText, Calendar, ExternalLink, Clock, CheckCircle, XCircle } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import apiClient from '../../api/client';
import ActionModals from '../../components/common/ActionModals';

export default function EmployeeDocumentsModuleScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/documents/employee?document_type=all`);
      ...
      console.warn('Error fetching employee documents:', error);
      setData([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Document', `Are you sure you want to delete ${item.document_name}?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/documents/employee/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      await apiClient.post('/documents/employee', updatedItem);
    } catch(err) {
  ...
  const getStatusInfo = (status) => {
    switch(status) {
      case 'Verified': return { bg: '#DCFCE7', text: '#15803D', icon: CheckCircle };
      ...
      <View style={styles.card}>
        ...
        <View style={styles.detailsGrid}>
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText} numberOfLines={1}>{item.document_type}</Text>
          </View>
          {item.expiry_date && (
            <View style={styles.detailItem}>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            ...
            <Text style={styles.headerSubtitle}>Track and verify records</Text>
          </View>
        ...
        <View style={styles.searchRow}>
          <View style={styles.searchBox}>
            ...
            <TextInput
              ...
              placeholder="Search by name, doc, or type..."
              value={search}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No employee documents found</Text>
            </View>
          }
        />
      )}
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Upload Document' : 'Details'}
        />
      )}
    </View>
  );
