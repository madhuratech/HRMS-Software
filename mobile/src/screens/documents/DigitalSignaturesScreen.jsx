...
import { Search, Plus, Trash2, FileSignature, Calendar, ExternalLink, Clock, CheckCircle } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import apiClient from '../../api/client';
import ActionModals from '../../components/common/ActionModals';

export default function DigitalSignaturesScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/documents/signatures`);
      ...
      console.warn('Error fetching signatures:', error);
      setData([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Request', `Are you sure you want to delete this signature request?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/documents/signatures/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      await apiClient.post('/documents/signatures', updatedItem);
    } catch(err) {
  ...
  const filteredData = data.filter(item => 
    ...
    (item.requested_to || '').toLowerCase().includes(search.toLowerCase())
  );
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.docName} numberOfLines={1}>{item.doc_name}</Text>
            <Text style={styles.requestedTo}>To: {item.requested_to}</Text>
          </View>
        </View>
        ...
        <View style={styles.detailsGrid}>
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText} numberOfLines={1}>
              Requested: {item.created_at ? new Date(item.created_at).toLocaleDateString('en-IN') : '--'}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Clock size={14} color="#64748B" />
            <Text style={styles.detailText} numberOfLines={1}>
              Expiry: {item.expiry_date ? new Date(item.expiry_date).toLocaleDateString('en-IN') : '--'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.footerRow}>
          <View style={[styles.statusBadge, { backgroundColor: isCompleted ? '#DCFCE7' : '#FEF3C7' }]}>
            {isCompleted ? <CheckCircle size={12} color="#15803D" /> : <Clock size={12} color="#D97706" />}
            <Text style={[styles.statusText, { color: isCompleted ? '#15803D' : '#D97706' }]}>
              {item.status || 'Pending'}
            </Text>
          </View>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            <Text style={styles.headerTitle}>Digital Signatures</Text>
            <Text style={styles.headerSubtitle}>Manage digital document signing</Text>
          </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No signature requests found</Text>
            </View>
          }
        />
      )}
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Request Signature' : 'Details'}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  requestedTo: { fontSize: 13, color: '#64748B' },
