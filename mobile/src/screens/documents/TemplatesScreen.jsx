...
import { Search, Plus, Trash2, FileText, Calendar, ExternalLink, PenTool } from 'lucide-react-native';
...
export default function TemplatesScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/documents/templates?category=all`);
      ...
      console.warn('Error fetching document templates:', error);
      setData([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Template', `Are you sure you want to delete ${item.template_name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/documents/templates/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      await apiClient.post('/documents/templates', updatedItem);
    } catch(err) {
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.templateName} numberOfLines={1}>{item.template_name}</Text>
        ...
        <View style={styles.detailsGrid}>
          ...
          <Text style={[styles.detailText, {width: '100%', marginTop: 6}]} numberOfLines={2}>
            {item.description}
          </Text>
        </View>
      ...
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            ...
            <Text style={styles.headerSubtitle}>Manage document templates & forms</Text>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  templateName: { fontSize: 15, fontWeight: '700', color: '#1E293B', marginBottom: 4 },
  templateCategory: { fontSize: 13, color: '#64748B' },
