...
import { Search, Plus, Edit2, Trash2, Calendar, CheckCircle, Clock, UserCheck, CheckSquare, XSquare } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import apiClient from '../../api/client';
import ActionModals from '../../components/common/ActionModals';

export default function ProbationScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/probations?page=1&limit=50`);
      if (res.data?.success && res.data?.data?.probations) {
        setData(res.data.data.probations);
      } else {
        setData([]);
      }
    } catch (error) {
      console.warn('Error fetching probations:', error);
      setData([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Record', `Are you sure you want to delete probation for ${item.employee}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/app/probations/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if (updatedItem.id) {
        await apiClient.put(`/app/probations/${updatedItem.id}`, updatedItem);
      } else {
        await apiClient.post('/app/probations', updatedItem);
      }
    } catch(err) {
  ...
  const handleComplete = (item) => {
    Alert.alert('Complete Probation', `Mark ${item.employee}'s probation as Completed?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Complete', onPress: async () => {
          try {
             await apiClient.post(`/app/probations/${item.id}/complete`);
             fetchData();
          } catch(err) {
             Alert.alert('Error', 'Failed to complete probation');
          }
      }}
    ]);
  };
  ...
  const getStatusInfo = (status) => {
    switch(status) {
      case 'Completed': return { bg: '#ECFDF5', text: '#10B981', icon: CheckCircle };
      case 'Extended': return { bg: '#FEF2F2', text: '#EF4444', icon: Clock };
      case 'In Progress': return { bg: '#EFF6FF', text: '#2952E3', icon: Clock };
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <View style={{ flex: 1, marginLeft: 12 }}>
            ...
            <Text style={styles.roleText}>{item.designation} • {item.department}</Text>
          </View>
        </View>
        ...
        <View style={styles.detailsGrid}>
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText} numberOfLines={1}>
              Start: {item.probation_start_date ? new Date(item.probation_start_date).toLocaleDateString('en-IN') : '--'}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Calendar size={14} color="#64748B" />
            <Text style={styles.detailText} numberOfLines={1}>
              End: {item.probation_end_date ? new Date(item.probation_end_date).toLocaleDateString('en-IN') : '--'}
            </Text>
          </View>
        </View>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            <Text style={styles.headerTitle}>Probation</Text>
            <Text style={styles.headerSubtitle}>Track probation periods & confirmations</Text>
          </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No probation records found</Text>
            </View>
          }
        />
      )}
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Add Probation' : 'Edit Probation'}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  roleText: { fontSize: 13, color: '#64748B' },
