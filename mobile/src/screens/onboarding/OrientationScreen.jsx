...
import { Search, Plus, Edit2, Trash2, CalendarDays, CheckCircle, Clock, Video, MonitorPlay } from 'lucide-react-native';
...
export default function OrientationScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/orientations?page=1&limit=50`);
      if (res.data?.success && res.data?.data?.orientations) {
        setData(res.data.data.orientations);
      } else {
        setData([]);
      }
    } catch (error) {
      console.warn('Error fetching orientations:', error);
      setData([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Cancel Session', `Are you sure you want to cancel ${item.title}?`, [
      ...
      { text: 'Cancel', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/app/orientations/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if (updatedItem.id) {
        await apiClient.put(`/app/orientations/${updatedItem.id}`, updatedItem);
      } else {
        await apiClient.post('/app/orientations', updatedItem);
      }
    } catch(err) {
  ...
  const getStatusInfo = (status) => {
    switch(status) {
      ...
      case 'Scheduled': return { bg: '#EFF6FF', text: '#2952E3', icon: CalendarDays };
        ...
        <View style={styles.cardHeader}>
          <View style={styles.iconContainer}>
            {item.session_type === 'Online' ? (
               ...
               <MonitorPlay size={20} color="#2563EB" />
            )}
          </View>
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.sessionTitle} numberOfLines={1}>{item.title}</Text>
            <Text style={styles.trainerText}>Trainer: {item.trainer}</Text>
          </View>
        </View>
        ...
        <View style={styles.detailsGrid}>
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText} numberOfLines={1}>
              {item.orientation_date ? new Date(item.orientation_date).toLocaleDateString('en-IN') : '--'}
            </Text>
          </View>
          <View style={styles.detailItem}>
             {item.session_type === 'Online' ? (
                 ...
                 <MonitorPlay size={14} color="#64748B" />
             )}
             <Text style={styles.detailText} numberOfLines={1}>{item.session_type || 'Offline'}</Text>
          </View>
        </View>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            ...
            <Text style={styles.headerSubtitle}>Schedule sessions & configure templates</Text>
          </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              <MonitorPlay size={40} color="#CBD5E1" />
              <Text style={styles.emptyText}>No orientation sessions found</Text>
            </View>
          }
        />
      )}
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Schedule Orientation' : 'Edit Session'}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  sessionTitle: { fontSize: 15, fontWeight: '700', color: '#1E293B', marginBottom: 4 },
  trainerText: { fontSize: 13, color: '#64748B' },
