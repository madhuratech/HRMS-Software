...
import { Search, Plus, Edit2, Trash2, Calendar, UserPlus, CheckCircle, Clock, Percent } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import apiClient from '../../api/client';
import ActionModals from '../../components/common/ActionModals';

export default function NewJoinersScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/app/joiners?page=1&limit=50`);
      if (res.data?.success && res.data?.data?.joiners) {
        setData(res.data.data.joiners);
      } else {
        setData([]);
      }
    } catch (error) {
      console.warn('Error fetching new joiners:', error);
      setData([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Record', `Are you sure you want to delete ${item.employee}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/app/joiners/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if (updatedItem.id) {
        await apiClient.put(`/app/joiners/${updatedItem.id}`, updatedItem);
      } else {
        await apiClient.post('/app/joiners', updatedItem);
      }
    } catch(err) {
  ...
  const getStatusInfo = (status) => {
    switch(status) {
      case 'Completed': return { bg: '#DCFCE7', text: '#15803D', icon: CheckCircle };
      case 'In Progress': return { bg: '#DBEAFE', text: '#1D4ED8', icon: Clock };
        ...
        <View style={styles.detailsGrid}>
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText} numberOfLines={1}>
              Joining: {item.joining_date ? new Date(item.joining_date).toLocaleDateString('en-IN') : '--'}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Percent size={14} color="#64748B" />
            <Text style={styles.detailText} numberOfLines={1}>
              Checklist: {item.checklist_completed || '0%'}
            </Text>
          </View>
        </View>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            <Text style={styles.headerTitle}>New Joiners</Text>
            <Text style={styles.headerSubtitle}>Manage employee onboarding process</Text>
          </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No new joiners found</Text>
            </View>
          }
        />
      )}
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Add New Joiner' : 'Edit New Joiner'}
        />
      )}
    </View>
  );
