...
import { Search, Plus, Edit2, Trash2, Laptop, Calendar, CheckCircle, Clock, HardDrive, RefreshCcw } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import apiClient from '../../api/client';
import ActionModals from '../../components/common/ActionModals';

export default function AssetAllocationScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/assets?page=1&limit=50`);
      if (res.data?.success && res.data?.data?.allocations) {
        setData(res.data.data.allocations);
      } else {
        setData([]);
      }
    } catch (error) {
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Asset', `Are you sure you want to delete ${item.asset_name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/app/assets/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if (updatedItem.id) {
        await apiClient.put(`/app/assets/${updatedItem.id}`, updatedItem);
      } else {
        await apiClient.post('/app/assets', updatedItem);
      }
    } catch(err) {
  ...
  const getStatusInfo = (status) => {
    switch(status) {
      case 'Allocated': return { bg: '#ECFDF5', text: '#10B981', icon: CheckCircle };
      case 'Pending': return { bg: '#FFFBEB', text: '#F59E0B', icon: Clock };
      case 'Returned': return { bg: '#F1F5F9', text: '#64748B', icon: RefreshCcw };
      ...
      <View style={styles.card}>
        ...
        <View style={styles.detailsGrid}>
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailText} numberOfLines={1}>{item.asset_type}</Text>
          </View>
          <View style={styles.detailItem}>
            <Calendar size={14} color="#64748B" />
            <Text style={styles.detailText} numberOfLines={1}>
              {item.allocation_date ? new Date(item.allocation_date).toLocaleDateString('en-IN') : '--'}
            </Text>
          </View>
        </View>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            ...
            <Text style={styles.headerSubtitle}>Manage IT assets and equipment</Text>
          </View>
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Allocate Asset' : 'Edit Allocation'}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  assetName: { fontSize: 15, fontWeight: '700', color: '#1E293B', marginBottom: 4 },
  employeeName: { fontSize: 13, color: '#64748B' },
