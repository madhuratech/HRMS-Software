...
import { Search, Plus, Edit2, Trash2, Package, Calendar, CheckCircle, Clock, Truck } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import apiClient from '../../api/client';
import ActionModals from '../../components/common/ActionModals';

export default function WelcomeKitScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.get(`/tickets/welcome-kits`);
      if (res.data) {
      ...
      console.warn('Error fetching welcome kits:', error);
      setData([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleActionDelete = (item) => {
    Alert.alert('Delete Kit', `Are you sure you want to delete kit for ${item.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
          try {
             await apiClient.delete(`/tickets/welcome-kits/${item.id}`);
             fetchData();
          } catch(err) {
  ...
  const handleActionSave = async (updatedItem) => {
    try {
      if (updatedItem.id) {
        await apiClient.put(`/tickets/welcome-kits/${updatedItem.id}`, updatedItem);
      } else {
        await apiClient.post('/tickets/welcome-kits', updatedItem);
      }
    } catch(err) {
  ...
  const getStatusInfo = (status) => {
    switch(status) {
      case 'Delivered': return { bg: '#ECFDF5', text: '#10B981', icon: CheckCircle };
      case 'Dispatched': return { bg: '#EFF6FF', text: '#2952E3', icon: Truck };
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <View style={{ flex: 1, marginLeft: 12 }}>
            ...
            <Text style={styles.deptText}>{item.dept || 'Department N/A'}</Text>
          </View>
        </View>
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#FFFFFF', '#F8FAFC']} style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            <Text style={styles.headerTitle}>Welcome Kits</Text>
            <Text style={styles.headerSubtitle}>Manage kit allocation & tracking</Text>
          </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No welcome kits found</Text>
            </View>
          }
        />
      )}
      ...
      {actionModalVisible && (
        <ActionModals
          ...
          title={actionModalMode === 'add' ? 'Allocate Kit' : 'Edit Kit Allocation'}
        />
      )}
    </View>
  );
